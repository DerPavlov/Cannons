...
public class CannonList {
	 ...
	 public CannonList(UserMessages userMessages, Config config){
	 ...
	 private void createCannon(UUID Id, CannonData cannon, Player player)
	 {
		 ...
		 if (player != null) player.sendMessage(message.cannonBuilt);
	 }
	...
	private boolean CheckExistingCannon( CannonData cannon ){
		...
		if (block.getType() == config.Cannon_material)
		{
			BlockFace reverse = cannon.face.getOppositeFace();
			//button in front
			if (!CheckAttachedButton(block,cannon.face))
			{
				return false;
			}
			...
			block = block.getRelative(cannon.face,1);
			//button backside
			if (!CheckAttachedButton(block,reverse)){
				return false;
			}
			//torch backside
			if (!CheckAttachedTorch(block)){
				return false;
			}
			//cannon ok
			return true;
